...
export function handleHelp(): CommandResult {
  const lines = [
    ...
    '',
    '**🎲 ダイス・判定**',
    '`/cc <技能名> [+N/-N] [secret] [target: @PL]` — 技能ロール（セッションのシステムに応じた判定）',
    '　例: `/cc 目星` | `/cc INT +1` | `/cc 回避 -2 secret`',
    '　※ 付記付き技能（例: 運転（自動車））は `/cc 自動車` または `/cc 運転` でも検索可',
    '　※ ボーナス/ペナルティ（+N/-N）は第7版のみ使用可',
    '　※ `target` 指定（KP専用）：対象PLのキャラクター技能値で判定、結果はKPのみに表示',
    ...
    '`/char refresh` — キャラクターシートを再取得して技能を更新（HP・SAN等の現在値は維持）',
    ...
    '`/session pc list` — 参加者の HP / MP / SAN / 幸運を一覧表示',
