...
```
/cc <技能名または能力値> [+N/-N] [secret] [target: @PL]
```
...
| `target` | （**KP専用**）対象 PL を @メンションで指定。そのPLのキャラクター技能値で判定する |

**KP ターゲット指定ロール**

KP が PL のキャラクター技能値を使って判定したい場合（例: 隠れた心理学判定）に使用する。
結果は KP にのみ表示される（ephemeral）。ダイスログには記録されない。

```
/cc 心理学 target: @プレイヤー名
```

**出力例（KP ターゲット指定）**
```
🎲 心理学 (探索者A: 55)
ベース出目：34（10の位: 30, 1の位: 4）
結果：34 ＞ レギュラー成功
```
...
> **注**: セッションが進行中のときのみ実行できる。登録と同時にセッション参加者として記録される。

#### `/char status` — ステータス確認

現在の HP / MP / SAN / 幸運をチャンネルに表示する。
...
セッション中に受けたダメージや回復を反映する。セッションが進行中のときのみ実行できる。
...
> シークレットダイス（`secret` 付き）のロールもレポートに含まれる。詳細ログには `（シークレット）` が付記される。

#### `/session pc` — 参加者ステータス一覧

セッション参加者のステータスや能力値を一覧表示する。KP が全体状況を把握するのに便利。

```
/session pc list          → 全参加者の HP / MP / SAN / 幸運を一覧表示
/session pc <能力値名>    → 指定した能力値を高い順にランキング表示
```

**使用可能な能力値名**（第7版）: `STR` `CON` `DEX` `APP` `POW` `SIZ` `INT` `EDU` `HP` `MP` `SAN` `LUCK`（`幸運` でも可）  
**第6版追加**: `IDEA`（`アイデア` でも可）、`KNOWLEDGE`（`知識` でも可）

**出力例（`/session pc list`）**
```
📋 セッション「呪われた村」参加者一覧

遠山 陽子 — HP: 9 / MP: 10 / SAN: 52 / 幸運: 65
田中 一郎 — HP: 12 / MP: 8 / SAN: 70 / 幸運: 45
```

**出力例（`/session pc DEX`）**
```
📊 セッション「呪われた村」DEX 一覧（高い順）
