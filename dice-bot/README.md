...
- **`/session start`**: セッションを開始し、以降のダイスログを記録する
  - Discord UI では `/session start` を選択すると `name:` フィールドが表示されるので、そこにセッション名を入力する
  - 例: `name:` フィールドに `呪われた村` と入力
- **`/session end`**: セッションを終了。記録されたログを集計し、Markdown形式のレポートファイルを生成してDiscordに添付・送信する

### ❓ ヘルプ

- **`/help`**: 全コマンドの使い方を表示する（自分のみに見えるEphemeralメッセージ）
...
スラッシュコマンドはDiscord APIに事前登録が必要。登録は**デプロイとは別の1回限りの操作**（コマンド定義を変更した場合も再実行が必要）。

### GitHub Actions から実行する（推奨）

1. GitHubリポジトリの **「Actions」** タブを開く
2. 左メニューから **「スラッシュコマンド登録」** を選択
3. **「Run workflow」** をクリック
   - `guild_id`: 特定サーバーのみ即時反映したい場合はDiscordサーバーIDを入力（空欄にするとグローバル登録）
4. ワークフローを実行すると `DISCORD_BOT_TOKEN` と `DISCORD_APPLICATION_ID` が自動的に使用される

> **ギルドID（サーバーID）の確認方法**: Discordの設定 → 「詳細設定」で「開発者モード」を有効にし、サーバー名を右クリック → 「IDをコピー」

- **グローバル登録**（全サーバー対象）: 反映まで最大1時間かかる
- **ギルド登録**（特定サーバーのみ）: 即時反映。開発・テスト中はこちら推奨
